...
export async function testScraper(): Promise<void> {
    ...
    if (result) {
        ...
        if (result.avatarUrl) {
            console.log("🖼️ Avatar found:", result.avatarUrl);
        } else {
            console.log("⚠️ Avatar not found, trying dedicated avatar scraper...");
            const avatarOnly = await scrapeTwitterAvatar("elonmusk");
            if (avatarOnly) {
                console.log("🖼️ Avatar found with dedicated scraper:", avatarOnly);
            }
        }
    } else {
        console.log("❌ Scraping failed - try a different approach");
    }
}
